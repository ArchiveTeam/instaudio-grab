...
from seesaw.tracker import GetItemFromTracker, PrepareStatsForTracker, UploadWithTracker, SendDoneToTracker
import shutil
import socket
import subprocess
import sys
...
LUA_SHA1 = get_hash(os.path.join(CWD, 'instaudio.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--timeout", "30",
            "--tries", "inf",
            "--domains", "instaudio.com",
            "--span-hosts",
            "--waitretry", "30",
            "--warc-file", ItemInterpolation("%(item_dir)s/%(warc_file_base)s"),
            "--warc-header", "operator: Archive Team",
            "--warc-header", "instaudio-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("instaudio-item: %(item_name)s")
        ]
        ...
        if item_type in ('audio'):
            wget_args.append('https://instaud.io/{}'.format(item_value))
        else:
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='instaudio'),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
